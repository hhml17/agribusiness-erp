...
### 8.1 Error de MIME Types en Azure Static Web Apps

**Síntomas:**
```
Refused to apply style from 'https://erp.agribusiness.com.py/assets/index-*.css'
because its MIME type ('text/html') is not a supported stylesheet MIME type
```

**Causa:**
- Configuración incorrecta de `base` en vite.config.ts
- Azure SWA devolviendo index.html como fallback para rutas de assets
...
export default defineConfig({
  ...
  base: '/'  // ✅ Archivos en la raíz (NO '/app')
})
```
...
```json
{
  "navigationFallback": {
    ...
    "exclude": [
      "/assets/*",
      "/images/*",
      "/*.{css,js,json,png,jpg,gif,svg,ico,woff,woff2,ttf,eot}"
    ]
  },
  "mimeTypes": {
...
# 2. Build con configuración correcta
cd app && npm run build

# 3. Verificar que index.html referencia assets correctamente
cat dist/index.html | grep -E '(href|src)="/assets'
# Debe mostrar: <link rel="stylesheet" href="/assets/index-[hash].css">
#               <script type="module" src="/assets/index-[hash].js">

# 4. Push y verificar deploy
git add . && git commit -m "fix: correct vite base config"
git push
```

### 8.2 Error ERR_BLOCKED_BY_CLIENT
...
**Causa:** Extensiones del navegador bloqueando recursos:
- AdBlock
- uBlock Origin
- Privacy Badger
- Ghostery
...
#### Opción 2: Whitelist del dominio
1. Clic en ícono de la extensión bloqueadora
2. "Desactivar en este sitio" o "Añadir a whitelist"
3. Recargar página (F5)

#### Opción 3: Desactivar extensión temporalmente
```
...
**Nota:** NO es un problema del código, es configuración del usuario final.

### 8.3 Error de Autenticación Azure AD

**Síntomas:**
```
AADSTS50011: The reply URL specified does not match the reply URLs
configured for the application
```

**Causa:** Redirect URI no configurado en Azure AD

**Solución:**
1. Azure Portal → Microsoft Entra ID → App registrations
2. Buscar aplicación por Client ID
3. Authentication → Add a platform → Single-page application
4. Agregar URIs:
   - `http://localhost:5173` (desarrollo)
   - `https://[nombre-swa].azurestaticapps.net` (staging)
   - `https://erp.agribusiness.com.py` (producción)
5. Save y esperar 5 minutos para propagación

### 8.4 Error de Conexión a Base de Datos
...
echo $DATABASE_URL
# Debe contener: sqlserver://[server]:1433;database=[db];encrypt=true

# 2. Test de conectividad
npx prisma db pull --force

# 3. Verificar firewall de Azure SQL
# Azure Portal → SQL Server → Networking → Firewall rules
# Agregar tu IP pública actual
```

### 8.5 Error Prisma Generate
